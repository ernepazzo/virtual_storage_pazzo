class Admin::UsersController < ApplicationController
  ...
  def data
    ...
    users.each do |user|
      action = ''
      action += "<a class='dropdown-item' href='/admin/users/#{user.id}/perfil' data-controller='turbo'><span class='bi bi-eye text-info'></span> Perfil</a>"  if show_html('show')
      action += "<a class='dropdown-item' href='/#{I18n.locale}/admin/users/#{user.id}/edit' data-controller='turbo'><span class='bi bi-pencil text-warning'></span> Editar</a>"  if show_html('edit')
      action += "<a class='dropdown-item' href='javascript:;' data-action='dataTable#delete' data-target='/admin/users/#{user.id}'><span class='bi bi-trash text-danger' data-action='dataTable#delete' data-target='/admin/users/#{user.id}'></span> Eliminar</a>" if show_html('delete')
