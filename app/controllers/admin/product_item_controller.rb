class Admin::ProductItemController < ApplicationController
  ...
  def data
    ...
    product_items.each do |product_item|
      action = ''
      action += "<a class='dropdown-item' href='#{admin_product_item_show_path(id: product_item.id)}' data-controller='turbo'><span class='bi bi-eye text-info'></span> Mostrar</a>" if show_html('show')
      action += "<a class='dropdown-item' href='#{admin_product_item_edit_path(id: product_item.id)}' data-controller='turbo'><span class='bi bi-pencil text-warning'></span> Editar</a>" if show_html('edit')
      action += "<a class='dropdown-item' href='javascript:;' data-action='admin#delete' data-target='#{admin_product_item_delete_path(id: product_item.id)}'><span class='bi bi-trash text-danger' data-action='admin#delete' data-target='#{admin_product_item_delete_path(id: product_item.id)}'></span> Eliminar</a>" if show_html('delete')
